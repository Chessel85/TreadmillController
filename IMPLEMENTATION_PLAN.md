...
- **Next task:** 2.1 — Session state machine
...
- [x] **1.2 Treadmill connect flow.** `navigator.bluetooth.requestDevice` flow with an accessible "scanning / found / connected / failed" state sequence (PDD Ref #1).
...
- **2026-08-08**: Completed 1.2. Wired the real connect flow per `docs/ble-findings.md`: `requestDevice({ filters: [{ namePrefix: 'iConsole' }], optionalServices: [...] })`, requesting the proprietary `0xFFF0` service plus standard FTMS (`0x1826`) and Device Information (`0x180a`) up front so later phases can use them without re-pairing. State sequence (status text + assertive announcement at each step): "Scanning…" → "Found `<name>`. Connecting…" → "Connected to `<name>`." or a failure message (distinguishes user-cancelled-picker from an actual connection error). Added a `gattserverdisconnected` listener that resets the UI back to "Not connected" and disables/clears mode selection, and a `navigator.bluetooth` feature check that disables the Connect button with an explanatory status if Web Bluetooth isn't available. `device`/`gattServer` are kept as module-level variables for Phase 2 (telemetry subscribe) and Phase 4 (control writes) to reuse. Not yet tested on-device in Bluefy — worth a real-device check early in Phase 2 once there's telemetry to see. Phase 1 is now complete; moving to Phase 2 (manual mode logging).
