...
- **Next task:** 1.2 — Treadmill connect flow (real-device connect attempt failed; error message was "undefined : undefined" — Bluefy doesn't reject with a standard DOMException, so error display was rewritten to handle any error shape; awaiting retest)
...
- [ ] **1.2 Treadmill connect flow.** `navigator.bluetooth.requestDevice` flow with an accessible "scanning / found / connected / failed" state sequence (PDD Ref #1). Code written; real-device test on the JTX Sprint 6 failed ("Connection failed. Please try again.") — diagnostics added, awaiting retest.
